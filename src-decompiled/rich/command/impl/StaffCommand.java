...
public class StaffCommand extends Command {
   ...
   public void execute(String var1, String[] var2) {
      ...
      switch (var4) {
         case "add":
            if (var2.length < 2) {
               this.logDirect("Использование: staff add <name>", class_124.field_1061);
               return;
            }

            String var13 = var2[1];
            if (StaffUtils.isStaff(var13)) {
               this.logDirect(String.format("Игрок %s уже в списке персонала!", var13), class_124.field_1061);
               return;
            }

            StaffUtils.addStaffAndSave(var13);
            this.logDirect(String.format("Игрок %s добавлен в персонал!", var13), class_124.field_1060);
            break;
         case "remove":
         case "del":
         case "delete":
            if (var2.length < 2) {
               this.logDirect("Использование: staff remove <name>", class_124.field_1061);
               return;
            }

            String var12 = var2[1];
            if (!StaffUtils.isStaff(var12)) {
               this.logDirect(String.format("Игрок %s не найден в списке персонала!", var12), class_124.field_1061);
               return;
            }

            StaffUtils.removeStaffAndSave(var12);
            this.logDirect(String.format("Игрок %s удален из персонала!", var12), class_124.field_1060);
            break;
         case "clear":
            int var11 = StaffUtils.size();
            StaffUtils.clearAndSave();
            this.logDirect(String.format("Список персонала очищен! Удалено: %d", var11), class_124.field_1060);
            break;
         case "list":
            int var7 = 1;
            if (var2.length > 1) {
            ...
            if (var8.isEmpty()) {
               this.logDirect("Список персонала пуст!", class_124.field_1061);
               return;
            }
            ...
            var9.display(() -> {
               ...
               class_5250 var2x = class_2561.method_43470("  §c● §f" + var1x);
               class_5250 var3x = class_2561.method_43470("§7Нажмите чтобы удалить §f" + var1x + " §7из персонала");
