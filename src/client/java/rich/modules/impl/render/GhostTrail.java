...
public class GhostTrail extends ModuleStructure {
   ...
   public void onTick(TickEvent var1) {
      if (mc.player != null) {
         ...
         boolean var4 = !mc.player.isOnGround();
         if (var4 && !this.wasInAir) {
            float var5 = mc.player.limbAnimator.getSpeed();
            float var6 = mc.player.limbAnimator.getAnimationProgress(1.0F) * (float) (Math.PI * 2);
            float var7 = (float)(Math.sin(var6) * 1.4F * var5);
            float var8 = (float)(-Math.sin(var6) * 1.4F * var5);
            float var9 = (float)(-Math.sin(var6) * 1.4F * var5 * 0.5) - (float)(Math.sin(mc.player.handSwingProgress * Math.PI) * 1.2F);
            ...
            this.ghost = new GhostTrail.Ghost(
               ...
               var10,
               var9,
               (float)Math.toRadians(mc.player.getPitch()),
               (float)Math.toRadians(mc.player.getHeadYaw() - mc.player.bodyYaw),
               var2,
   ...
   public void onWorldRender(WorldRenderEvent var1) {
      if (mc.player != null && this.ghost != null) {
         long var2 = System.currentTimeMillis();
         float var4 = (float)(var2 - this.ghost.spawnTime()) / this.ghost.fadeDurationMs();
         float var5 = 1.0F - var4 * var4;
         if (!(var5 <= 0.01F)) {
            int var6 = ColorUtil.a(this.color.getColor(), var5);
            int var7 = this.fillSetting.isValue() ? ColorUtil.a(var6, 0.25F) : 0;
   ...
   private void box(
      ...
      Box var73 = new Box(var38, var40, var42, var44, var46, var48);
      if (var29) {
   ...
   private record Ghost(double x, double y, double z, float yaw, float lLeg, float rLeg, float lArm, float rArm, float headPitch, float headYaw, long spawnTime, float fadeDurationMs) {
   }
}
