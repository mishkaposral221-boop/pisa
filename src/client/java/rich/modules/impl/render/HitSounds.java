...
public class HitSounds extends ModuleStructure implements IMinecraft {
   ...
   private void loadSounds() {
      ...
      this.availableSounds.add(new HitSounds.Sound("Arrow Hit", SoundEvents.ENTITY_ARROW_HIT_PLAYER));
      this.availableSounds.add(new HitSounds.Sound("Anvil Land", SoundEvents.BLOCK_ANVIL_LAND));
      this.availableSounds.add(new HitSounds.Sound("Crit", SoundEvents.ENTITY_PLAYER_ATTACK_CRIT));
   }

   @EventHandler
   public void onAttack(AttackEvent var1) {
      if (mc.player != null && mc.world != null) {
         if (var1.getTarget() instanceof LivingEntity) {
            if (!this.critOnly.isValue() || var1.isCrit()) {
               ...
               this.availableSounds.stream().filter(var1x -> Objects.equals(var1x.name(), var2)).findFirst().ifPresent(var1x -> {
                  ...
                  } else if (var1x.soundSource() instanceof SoundEvent var3) {
                     mc.player.playSound(var3, this.volume.getValue(), this.pitch.getValue());
                  }
               });
            }
         }
   ...
   private void playWav(String var1, float var2, float var3) {
      new Thread(() -> {
         try {
            InputStream var2x = mc.getResourceManager().open(Identifier.of("rich", "sounds/" + var1));
   ...
   private record Sound(String name, Object soundSource) {
   }
}
