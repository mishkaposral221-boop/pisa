...
public class ConfigSystem {
   ...
   public void save() {
      if (this.initialized.get()) {
         if (this.saving.compareAndSet(false, true)) {
            try {
               String var1 = this.serializer.serialize();
               boolean var2 = this.fileHandler.write(var1);
               if (var2) {
               ...
               Logger.error("AutoConfiguration: Save error! " + var6.getMessage());
            } finally {
   ...
   public void load() {
      if (!this.fileHandler.exists()) {
         ...
         try {
            String var1 = this.fileHandler.read();
            if (var1 != null && !var1.isEmpty()) {
               this.serializer.deserialize(var1);
               Logger.success("AutoConfiguration: autoconfig.json loaded successfully!");
            }
         } catch (Exception var2) {
            Logger.error("AutoConfiguration: Load error! " + var2.getMessage());
         }
      }
   }
